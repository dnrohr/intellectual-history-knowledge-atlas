...
- [x] Add "why this link?" evidence explanations.
- [x] Keep the link review queue as a low-confidence exception path.
...
- [x] Add a thread viewer that steps through a chain one relationship at a time.
- [x] Add thread-aware graph highlighting:
...
- [x] Revisit minimap/overview behavior for dense graph states after typed entities expand the graph.
...
- [x] Add snapshot tests for canonical data output.
- [x] Add CI checks that fail on impossible dates, invalid references, self-links, and schema errors.
